    ...
    routes() {
        ...
        router.put("/media/stage/:system?", async (req, res) => {
            ...
            try {
                ...
                let modifier = {
                    $set: {
                        system: req.params.system, origin: origin, status: "staged", _modified: new Date()
                    }, $setOnInsert: {
                        _created: new Date()
                    }
                }
                if (origin === 'upload') {
                    Object.assign(modifier.$set, {
                        file: req.body._id + '.' + ext, type: req.body.type, size: req.body.size,
                    })
                } else if (origin === 'url') {
                    Object.assign(modifier.$set, {
                        file: req.body._id + '.png', type: 'image/png', url: req.body.url
                    })
                }
